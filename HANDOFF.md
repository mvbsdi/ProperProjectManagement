...
| `assistant.js` | **Timmy the Tote** (v2) — interactive assistant included by every page (`<script defer src="assistant.js">`). First visit: asks role (8 roles), spotlights that role's starting doc, offers onboarding. **Clicking him opens a menu**: *Onboard me* (tracked, resumable doc tour — Execs get exec deck → tracker; everyone else gets exec deck FIRST ("the candid one" — wins buy-in) → team deck → swimlane → tracker; if someone fizzles mid-tour he offers to resume ONCE per browser session, never nags), *Walk me through this page* (step-by-step spotlight walkthrough of the actual sections/fields on every page — content in the `WALKS` dict, 8th-grade voice), *Where do I start?* (role routing). **Stays out of the way**: fades to a ghost while any input/textarea/select is focused; one passive tip per page shown once. × = session hide, "don't show again" = permanent, "need a hand?" edge tab re-summons. Avatars: Timmy the Tote (red AutoStore bin, default) / Robert the Robo-Arm, both inline SVG. State: localStorage `ppm_assistant` `{off, role, avatar, seen, tour, tourDone}`. Walkthrough spotlight selectors are verified against the real pages (`.flow`, `.gate-tabs`, `.section`, `#list`, `.legend`, `table`, `.filters`) — keep them in sync if page structure changes. **Give feedback**: menu item opens a form in his bubble (message + optional name, page and role attached automatically) → POST `api/feedback` → stored server-side as `feedback.jsonl` in the private `mvbsdi/ppm-projects` repo (git-pushed per entry). Mike reviews there, or authenticated GET on `/ppm/api/feedback`. Draft survives a failed send; empty sends get an inline nudge. |
